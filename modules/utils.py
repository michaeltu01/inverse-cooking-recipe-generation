...
# CHANGED: retry mechanism for saves in tf
def tensorflow_persistent_save(model, filepath):
    for i in range(3):
        try:
            model.save(filepath)
            break
            ...
            if i == 2:
                ...
                logging.info("Attempt {} failed, retrying...".format(i+1))
...
def save_state(filename, args, model, criterion, optimizer, lr_scheduler,
    ...
    tensorflow_persistent_save(state_dict, filename)


def load_model_state(filename, model):
    if not os.path.exists(filename):
        return None, [], None
    #CHANGED: removed states to align with saving models in tf
...
def make_positions(tensor, padding_idx, left_pad):
    ...
    if make_positions.range_buf.numel() < max_pos:
        tf.arange(padding_idx + 1, max_pos, out=make_positions.range_buf)
